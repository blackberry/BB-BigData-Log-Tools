...
public class IndexLogs {
  ...
  private static void findComponents(Map<String, Map<String, Map<String, Map<String, Component>>>> data, List<String> unmergedCSVStrings, FileSystem fs, Path path) throws FileNotFoundException, IOException, ParseException {
  ...
  private static void updateComponent(Map<String, Map<String, Map<String, Map<String, Component>>>> data, List<String> unmergedCSVStrings, FileSystem fs, FileStatus matchedFolder, Path path) throws IOException, ParseException {
    ...
    if (!fs.exists(READYPath)) {
      unmergedCSVStrings.add(DC + "," + service + "," + type + "," + component + "," + pathPieces[pathPieces.length - 4] + "," + pathPieces[pathPieces.length - 3] + "\n");
  ...
  public static void main(String args[]) throws IOException, ParseException {
    ...
    for (int i = 0; i < args.length; i++) {
      if(args[i].matches("-t")) {
        ...
        System.out.println("Usage: indexlogs [-t -n -r -f]\n    -t      Print results to STDOUT in human-readable tree\n" +
                                                     "    -n      Don't write index files into HDFS\n" +
    ...
    if(forceRemove && !removeOldIndexes) {
      System.out.println("Warning: Asked to force-remove (-f) indexes without removing old index files (-r). Aborting.");
      System.exit(0);
    }
    ...
    if (writeIndex) {
      ...
      outputCSV.close();
      FSDataOutputStream outputJSON = fs.create(new Path("/service/.index/logindex." + currentTime + ".json"));
      writeJSON(data, outputJSON);
      outputJSON.close();
      System.out.println("Index files written to /service/.index/logindex." + currentTime + ".csv and /service/.index/logindex." + currentTime + ".json");
      FSDataOutputStream unmergedCSV = fs.create(new Path("/service/.index/unmerged." + currentTime + ".csv"));

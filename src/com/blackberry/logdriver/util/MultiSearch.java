...
import com.blackberry.logdriver.mapreduce.avro.AvroBlockInputFormat;
...
public class MultiSearch extends Configured implements Tool {
  ...
  static boolean ANDsearch = false;
  static boolean ORsearch = true;
  static boolean caseSensitive = false;
  static boolean caseInsensitive = true;
    ...
    protected void add(byte[] bytes, int i) {
      ...
      try {
        if (caseSensitive) {
          ...
          child = children.get(new String(bytes, "UTF-8").toLowerCase().getBytes("UTF-8")[i]);
          if (child == null) {
            child = children.get(new String(bytes, "UTF-8").toUpperCase().getBytes("UTF-8")[i]);
          } 
        }

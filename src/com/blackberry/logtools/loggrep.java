...
import com.blackberry.logdriver.util.GrepByTime;

public class loggrep extends Configured implements Tool {
	...
	public static void main(String[] argv) throws Exception {
		...
		int res = ToolRunner.run(new Configuration(), new loggrep(), argv);
		System.exit(res);
	}  
	...
	public int run(String[] argv) throws Exception {	
		...
		String tmp = "tmp/loggrep-" + RandomStringUtils.randomAlphanumeric(10);
		
		//Set args[outNum] to be temp output directory to be passed onto GrepByTime instead of UserInput argument
		args[outNum] = (StringEscapeUtils.escapeJava(tmp) + "/rawlines");

		//Managing console output - deal with --v/--silent
		Logger LOG = LoggerFactory.getLogger(loggrep.class);
		...
		LogTools.logConsole(quiet, silent, warn, "Searching for " + args[regexNum] + "...");
		LogTools.logConsole(quiet, silent, warn, "Passing Arguments: Regex=" + args[regexNum] + " DC=" + args[dcNum] + " Service=" + args[svcNum]
		...
		//Start Mapreduce job
		tools.runMRJob(quiet, silent, conf, D_options, out, LOG, field_separator, queue_name, args, "GrepByTime", new GrepByTime());
